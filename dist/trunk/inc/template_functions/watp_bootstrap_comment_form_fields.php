...
if ( ! function_exists( 'watp_bootstrap_comment_form_fields' ) ) {
	/**
	 * Creates the comments form.
	 *
	 ...
	 * @return array
	 */
	function watp_bootstrap_comment_form_fields( $fields ) {
		...
		$fields    = array(
			'author'  => '<div class="form-group comment-form-author"><label for="author">' . __( 'Name',
					'watp' ) . ( $req ? ' <span class="required">*</span>' : '' ) . '</label> ' .
			...
			'cookies' => '<div class="form-group form-check comment-form-cookies-consent"><input class="form-check-input" id="wp-comment-cookies-consent" name="wp-comment-cookies-consent" type="checkbox" value="yes"' . $consent . ' /> ' .
			         '<label class="form-check-label" for="wp-comment-cookies-consent">' . __( 'Save my name, email, and website in this browser for the next time I comment', 'watp' ) . '</label></div>',
		);

		return $fields;
	}
} // endif function_exists( 'watp_bootstrap_comment_form_fields' )

// Comments form.
add_filter( 'comment_form_default_fields', 'watp_bootstrap_comment_form_fields' );